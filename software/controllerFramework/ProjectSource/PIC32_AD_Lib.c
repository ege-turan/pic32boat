...
bool ADC_ConfigAutoScan(uint16_t whichPins)
{
    ...
    else // OK, looks good
    {
        ...
        AD1CON1bits.CLRASAM =
            0; // 0 = buffer contents will be overwritten by the next conversion sequence
        AD1CON1bits.SSRC =
            0b111; // 111 = Internal counter ends sampling and starts conversion (auto convert)
...
void ADC_MultiRead(uint32_t *adcResults)
{
    ...
    for (i = 0; i < numChanInSet; i++)
    {
        ...
        LocalResult = adcResults[i] =
            *(resultSet + (4 * i)); // read the results from the ADC1BUFx registers
    }
...
static uint8_t CountBits(uint16_t num2Count)
{
    ...
    #pragma config PMDL1WAY =                                                                      \
        OFF // Peripheral Module Disable Configuration (Allow multiple reconfigurations)
    #pragma config IOL1WAY =                                                                       \
        OFF // Peripheral Pin Select Configuration (Allow multiple reconfigurations)
    ...
    #pragma config FCKSM =                                                                         \
        CSDCMD // Clock Switching and Monitor Selection (Clock Switch Disable, FSCM Disabled)
    #pragma config WDTPS = PS1048576 // Watchdog Timer Postscaler (1:1048576)
    #pragma config WINDIS =                                                                        \
        OFF // Watchdog Timer Window Enable (Watchdog Timer is in Non-Window Mode)
...
void main(void)
{
    ...
    {
        printf(
            "ADC_ConfigAutoScan() failed when requesting too many channels (correct behavior)\n\r");
    }
    else
    {
    }
