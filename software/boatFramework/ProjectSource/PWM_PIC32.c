...
static uint32_t volatile *ChannelTo_pTimer[MAX_NUM_CHANNELS] = {&PR2, &PR2, &PR2, &PR2, &PR2};

// the lists of legal pin numbers for the output channels
static PWM_PinMap_t const LegalOutPins[][5] = {{PWM_RPA0, PWM_RPB3, PWM_RPB4, PWM_RPB7, PWM_RPB15},
...
static volatile uint32_t *const outputMapRegisters[] = {
    &RPA0R, &RPA1R, &RPA2R, &RPA3R, &RPA4R,  &RPB0R,  &RPB1R,  &RPB2R,  &RPB3R,  &RPB4R, &RPB5R,
    &RPB6R, &RPB7R, &RPB8R, &RPB9R, &RPB10R, &RPB11R, &RPB12R, &RPB13R, &RPB14R, &RPB15R};

// these are the constants used to map OCx channel constants to output pins
static uint32_t const mapChannel2PinSelConst[] = {
    0b0101 /*OC1*/, 0b0101 /*OC2*/, 0b0101 /*OC3*/, 0b0101 /*OC4*/, 0b0110 /*OC5*/
};

static PWM_PinMap_t const LegalOC2OutPins[] = {PWM_RPA1, PWM_RPB1, PWM_RPB5, PWM_RPB8, PWM_RPB11};

static PWM_PinMap_t const LegalOC3OutPins[] = {PWM_RPA3, PWM_RPB0, PWM_RPB9, PWM_RPB10, PWM_RPB14};

static PWM_PinMap_t const LegalOC4_5OutPins[] = {PWM_RPA2, PWM_RPA4, PWM_RPB2, PWM_RPB6, PWM_RPB13};
...
bool PWMOperate_SetPulseWidthOnChannel(uint16_t NewPW, uint8_t whichChannel)
{
    ...
    if (IsChannelIllegal(whichChannel) || (NewPW > *ChannelTo_pTimer[whichChannel - 1]))
    {
        ReturnVal = false;
    }
    else
...
bool PWMSetup_SetFreqOnTimer(uint16_t reqFreq, WhichTimer_t WhichTimer)
{
    ...
    if (((_Timer2_ != WhichTimer) && (_Timer3_ != WhichTimer)) || (MIN_FREQ > reqFreq) ||
        (MAX_FREQ < reqFreq))
    {
        ReturnVal = false;
    }
