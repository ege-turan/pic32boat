...
bool IC_Setup_ConfigureTimer(WhichTimer_t whichTimer,
                             uint32_t periodTicks,
                             IC_Prescaler_t prescaler)
{
...
{
    if (channel < IC1 || channel > IC5)
        return false;

    // Table of ICxCON registers
    static volatile void *const ICxCON_Table[5] = {&IC1CON, &IC2CON, &IC3CON, &IC4CON, &IC5CON};
...
void IC_ClearErrorFlag(IC_Channel_t channel)
{
    switch (channel)
    {
        ...
        case IC2:
            ...
            break;
        case IC3:
            IFS0CLR = _IFS0_IC3EIF_MASK;
            break;
        case IC4:
            IFS0CLR = _IFS0_IC4EIF_MASK;
            break;
        case IC5:
            IFS0CLR = _IFS0_IC5EIF_MASK;
            break;
    }
}
