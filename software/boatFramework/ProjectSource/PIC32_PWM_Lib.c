...
static volatile uint32_t *ChannelTo_pTimer[MAX_NUM_CHANNELS] = {&PR2, &PR2, &PR2, &PR2, &PR2};
...
static uint32_t const mapChannel2PinSelConst[] = {0b0101, 0b0101, 0b0101, 0b0101, 0b0110};
...
****************************************************************************/
bool PWM_Setup_ConfigureTimer(WhichTimer_t whichTimer,
                              uint32_t periodTicks,
                              PWM_Prescaler_t prescaler)
{
    if (prescaler > PWM_PS_256)
        return false; // invalid prescaler

    if (periodTicks < 1 || periodTicks > 0x10000)
        return false; // invalid number of ticks
...
****************************************************************************/
bool PWM_Setup_AssignChannelToTimer(uint8_t whichChannel, WhichTimer_t whichTimer)
{
    uint8_t idx;
...
bool PWM_Operate_SetDutyOnChannel(uint8_t dutyCycle, uint8_t whichChannel)
{
    ...
    if ((0 == whichChannel) || (whichChannel > MAX_NUM_CHANNELS) || (dutyCycle > 100))
    {
        return false;
    }
    ...
    else if (100 == dutyCycle)
    {
        ...
        if (updateVal == 0)
            updateVal = (*(ChannelTo_pTimer[idx])); // to handle edge case of PR = 0xFFFF overflow
    }
    else
    {
