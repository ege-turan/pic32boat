...
#define PWM_PERIOD_TICKS ((F_PBCLK / (F_PWM_HZ * prescaler)) - 1) // in ticks, 250Hz with prescaler 2 (Value for PRy)
...
*/
void _InitMotorPWM();
void _SetDutyCycleFromThrottleAndDirection(uint32_t Throttle, uint32_t Direction); // Duty Cycle note: 50 is reverse, 75 is stopped, 100 is forward
      ...
      case ES_TIMEOUT:
      {
        if (ThisEvent.EventParam == DRIVING_TIMER)
        {
          ...
          if (CurrentThrottle > ThrottleResolution) {
            toggle = 1;; // wrap around to 0 after reaching max
          }
...
void _SetDutyCycleFromThrottleAndDirection(uint32_t Throttle, uint32_t Direction)
{
  ...
  const float RANGE = 25.0f;       // max adjustment from neutral
    ...
    float throttleOffset = ((float)Throttle - (float)ThrottleMidPoint) * RANGE / (float)ThrottleMidPoint;

    // Convert direction to signed range: -25 to +25
    float directionOffset = ((float)Direction - (float)DirectionMidPoint) * RANGE / (float)DirectionMidPoint;
